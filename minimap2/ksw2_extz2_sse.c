	...
	for (r = 0, last_st = last_en = -1; r < qlen + tlen - 1; ++r) {
		...
		if (!(flag & KSW_EZ_GENERIC_SC)) {
			for (t = st0; t <= en0; t += 16) {
				...
				sq = simde_mm_loadu_si128((__m128i*)&sf[t]);
				st = simde_mm_loadu_si128((__m128i*)&qrr[t]);
...
#endif
				simde_mm_storeu_si128((__m128i*)((uint8_t*)s + t), tmp);
			}
		} else {
			for (t = st0; t <= en0; ++t)
		...
		if (!approx_max) { // find the exact max with a 32-bit score array
			...
			if (r > 0) {
				...
				simde_mm_storeu_si128((__m128i*)HH, max_H_);
				simde_mm_storeu_si128((__m128i*)tt, max_t_);
				for (i = 0; i < 4; ++i)
