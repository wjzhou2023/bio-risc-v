...
static void multiseedSearchWorker(void *vp,int core) {
...
static void multiseedSearchWorker_2p5(void *vp,int core) {
	set_thread_affinity(core);
	//int tid = *((int*)vp);
	thread_tracking_pair *p = (thread_tracking_pair*) vp;
	int tid = p->tid;
...
static void steal_threads(int pid, int orig_nthreads, EList<int>& tids, EList<T*>& threads)
{
	...
	if(in_use < ncpu) {
		...
		threads.push_back(new T(multiseedSearchWorker, (void*)&tids.back(),tids.size() - 1));
	...
	OutFileBuf *metricsOfb)
{
	...
	all_threads_done = 0;
	{
		...
		for(int i = 0; i < nthreads; i++) {
			...
			if(bowtie2p5) {
				threads.push_back(new std::thread(multiseedSearchWorker_2p5, (void*)&tps[i],i));
			} else {
				threads.push_back(new std::thread(multiseedSearchWorker, (void*)&tps[i],i));
			}
			threads[i]->detach();
			SLEEP(10);
		}
